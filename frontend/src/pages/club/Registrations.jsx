...
export default function ClubRegistrations() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            [...(regData.individuals || []), ...(regData.teams || [])].forEach(r => {
                if (r.eventId && r.eventTitle) eventMap[r.eventId] = { id: r.eventId, title: r.eventTitle };
            });
    ...
    const handleExport = () => {
        if (regSubTab === 'individual') {
            ...
            exportToExcel(dataToExport, `Club_Registrations_Individuals`);
        } else {
            const exportData = [];
            ...
            exportToExcel(exportData, `Club_Registrations_Teams`);
        }
    };
    ...
    }, [allRegistrations.individuals, regEventFilter, statusFilter]);
    ...
    }, [allRegistrations.teams, regEventFilter, statusFilter]);
    ...
    return (
        <div className="space-y-6 animate-fadeIn pb-12 relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between relative z-10">
                <div>
                    <h1 className="text-3xl font-display font-black text-white italic uppercase tracking-tighter">Club Registrations</h1>
                    <p className="text-slate-400 text-sm mt-1">All event registrations for your club.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={fetchData} disabled={loading}
                        className="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold transition-all disabled:opacity-50 flex items-center gap-2">
                        {loading ? '⏳...' : '🔄 Refresh'}
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 items-center relative z-10">
                <select value={regEventFilter} onChange={e => setRegEventFilter(e.target.value)}
                    className="bg-slate-800 text-slate-300 rounded-xl px-3 py-2 border border-slate-700 text-xs focus:outline-none focus:border-purple-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
                {[
                    { label: 'Individual', value: allRegistrations.individuals.filter(r => !r.teamName).length, color: 'indigo' },
                    { label: 'Teams', value: allRegistrations.teams.length, color: 'purple' },
                    { label: 'Verified', value: allRegistrations.individuals.filter(r => !r.teamName && ['VERIFIED','PAID','FREE'].includes((r.status||'').toUpperCase())).length, color: 'emerald' },
                    { label: 'Pending', value: allRegistrations.individuals.filter(r => !r.teamName && (r.status||'PENDING').toUpperCase() === 'PENDING').length, color: 'amber' },
                ].map(card => (
                    <div key={card.label} className={`bg-${card.color}-500/10 border border-${card.color}-500/20 rounded-2xl p-4`}>
                        <p className={`text-${card.color}-400 text-2xl font-black`}>{card.value}</p>
                        <p className="text-slate-400 text-xs font-semibold mt-1">{card.label}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="flex gap-1 p-1 bg-slate-800/50 rounded-2xl border border-slate-700/50 w-fit relative z-10">
                {[{ id: 'individual', label: 'Individual', icon: '👤' }, { id: 'team', label: 'Teams', icon: '👥' }].map(t => (
                    <button key={t.id} onClick={() => setRegSubTab(t.id)}
                        className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold transition-all ${regSubTab === t.id ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
            ...
            {loading ? (
                ...
                <div className="bg-slate-800/30 rounded-[2rem] border border-slate-700/50 overflow-hidden relative z-10">
                    <div className="grid grid-cols-5 gap-4 px-6 py-3 bg-slate-800/80 border-b border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="col-span-2">Student</span><span>Event</span><span>Status</span><span>Action</span>
                    </div>
                ...
                <div className="bg-slate-800/30 rounded-[2rem] border border-slate-700/50 overflow-hidden relative z-10">
                    <div className="grid grid-cols-5 gap-4 px-6 py-3 bg-slate-800/80 border-b border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="col-span-2">Team</span><span>Event</span><span>Status</span><span>Members</span>
                    </div>
                    <div className="divide-y divide-slate-700/30">
                        {filteredTeams.length === 0 ? (
                            ...
                            <div key={t.id || i} className="flex flex-col">
                                <div 
                                    ...
                                    className="grid grid-cols-5 gap-4 px-6 py-4 hover:bg-slate-700/20 transition-colors items-center cursor-pointer"
                                >
