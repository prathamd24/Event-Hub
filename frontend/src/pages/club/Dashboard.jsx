...
export default function ClubDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch gallery
      try {
        const galleryRes = await api.get(`/api/club/gallery${q}`);
        setGallery(galleryRes.data?.gallery || []);
      } catch (e) { /* gallery may not exist yet */ }
            ...
            <form onSubmit={handleUpdateSettings} className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <input 
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          await api.post('/api/club/branding', formData, { headers: { 'Content-Type': undefined } });
              ...
              <div className="pt-6 border-t border-slate-800">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <label className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Club Gallery</label>
                    <p className="text-slate-600 text-xs mt-0.5">Add photos to showcase your club (max 10)</p>
                  </div>
                  <span className="text-[10px] text-slate-500 font-bold bg-slate-800 px-2 py-0.5 rounded-lg border border-slate-700/50">{gallery.length}/10</span>
                </div>
                ...
                {gallery.length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                    {gallery.map((url, idx) => (
                      <div key={idx} className="relative group rounded-xl overflow-hidden border border-slate-700 aspect-video">
                        <img 
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <button
                            onClick={async () => {
                              ...
                              try {
                                await api.delete(`/api/club/gallery/${idx}`);
                            ...
                            className="px-3 py-1.5 rounded-lg bg-red-500/80 hover:bg-red-500 text-white text-xs font-bold transition-all flex items-center gap-1.5"
                          >
                            <span>✕</span> Remove
                          </button>
                        </div>
                        <div className="absolute top-2 left-2">
                          <span className="text-[9px] bg-black/60 text-white backdrop-blur-sm px-2 py-0.5 rounded-full font-bold">{idx + 1}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          const res = await api.post('/api/club/gallery', formData, { headers: { 'Content-Type': undefined } });
                    ...
                    <label 
                      ...
                      className={`flex items-center justify-center gap-2 w-full py-4 rounded-xl border-2 border-dashed border-slate-700 hover:border-indigo-500 text-slate-400 hover:text-indigo-400 cursor-pointer transition-all ${galleryUploading ? 'opacity-50 cursor-wait' : ''}`}
                    >
                      {galleryUploading ? (
                        <><div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" /> Uploading...</>
                      ) : (
                        <><span className="text-lg">📸</span> <span className="text-xs font-bold">Add Gallery Photo</span></>
                      )}
                    </label>
                  </div>
                )}
