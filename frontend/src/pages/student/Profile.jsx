...
export default function StudentProfile() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isMissingCollege && (selectedCollegeId || collegeSearch)) {
                if (selectedCollegeId) data.append('collegeId', selectedCollegeId);
                else data.append('collegeNameManual', collegeSearch);
            } else if (isMissingCollege && !selectedCollegeId && !collegeSearch) {
                        ...
                        {!isEditing ? (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 gap-6 pt-4 border-t border-slate-700/50">
                                    ...
                                    {/* College Input - Readonly for regular, Dropdown for missing */}
                                    {!isMissingCollege ? (
                                        <div>
                                            <label className="block text-sm font-medium text-slate-400 mb-1">College <span className="text-xs text-slate-500 ml-2">(Read-only)</span></label>
                                            <input type="text" value={user?.collegeName || ''} readOnly className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-indigo-400/70 cursor-not-allowed" />
                                        </div>
                                    ) : (
