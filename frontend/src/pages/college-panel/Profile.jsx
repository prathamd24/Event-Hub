...
export default function CollegeProfile() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Reset files state (images are now saved on server)
            setFiles({ logo: null, banner: null });

            // Update preview with server URLs
            setPreview({
                logo: college.logoUrl ? (college.logoUrl.startsWith('http') ? college.logoUrl : `${BACKEND_URL}${college.logoUrl}`) : preview.logo,
                banner: college.bannerUrl ? (college.bannerUrl.startsWith('http') ? college.bannerUrl : `${BACKEND_URL}${college.bannerUrl}`) : preview.banner,
            });
        } catch (error) {
            console.error('Profile update error:', error.response?.data);
            toast(error.response?.data?.error || error.response?.data?.message || 'Failed to update profile', 'error');
        } finally {
            setSubmitting(false);
        }
    };
