...
export default function CreateClubModal({ isOpen, onClose, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Send first coordinator with the field names the backend expects
            const primary = coordinators[0];
            data.append('facultyName', primary.name.trim());
            data.append('facultyEmail', primary.email.trim().toLowerCase());
            data.append('coordinatorPassword', primary.password.trim());

            // Also send remaining coordinators as JSON for future multi-coordinator support
            if (coordinators.length > 1) {
                data.append('extraCoordinators', JSON.stringify(coordinators.slice(1)));
            }
